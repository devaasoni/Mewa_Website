...
function initActivePageHighlight() {
  // 1. Get the current page name from the URL (e.g., "about-mewa.html")
  const path = window.location.pathname;
  let page = path.split("/").pop();

  // 2. Handle the root URL (e.g., website.com/) which should map to index.html
  if (page === "" || page === "/") {
    page = "index.html";
  }

  // 3. Select all navigation links (Desktop & Mobile)
  ...
  navLinks.forEach(link => {
    ...
    // 4. Clean the href to ignore anchors (e.g., "about-mewa.html#faq" becomes "about-mewa.html")
    const cleanHref = href.split('#')[0].split('?')[0];

    // 5. Compare and add class
    // We decodeURIComponent to handle special chars like '&' in "refer&earn.html"
    if (cleanHref === decodeURIComponent(page)) {
      link.classList.add('active');
    }
  });

  // 6. Special Handling for Mobile Dropdown Buttons (About & App)
  // Since mobile dropdown toggles are <button> tags without hrefs, we highlight them manually based on the page.
...
function highlightMobileDropdown(textToFind) {
  ...
  buttons.forEach(btn => {
    if (btn.textContent.trim().includes(textToFind)) {
      btn.classList.add('active');
      // Optional: Make the text gradient by targeting the inner text node if needed, 
      // but the .active CSS class should handle the button color.
      btn.style.color = "var(--gradient-2)"; // Fallback for buttons if gradient text doesn't apply nicely
    }
  });
}